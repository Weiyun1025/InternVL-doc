...
| InternVL2-8B-RPO | MLLM | 8.1B  | 🤗 [HF link](https://huggingface.co/OpenGVLab/InternVL2-8B-RPO) | 16 GB |
...
# Download OpenGVLab/InternVL2-8B-RPO
huggingface-cli download --resume-download --local-dir-use-symlinks False OpenGVLab/InternVL2-8B-RPO --local-dir InternVL2-8B-RPO
```
...
## Prepare Our MM-Reasoning Dataset
To prepare the training data, please first download our [MM-Reasoning dataset](https://huggingface.co/datasets/OpenGVLab/MM-Reasoning) and [the JSON file](https://huggingface.co/datasets/OpenGVLab/MM-Reasoning/blob/main/meta.json).

Our dataset contains approximately 3.4 million preference pairs, of which only around 1.0 million are utilized during training. You can adjust the number of active data samples and the data mixture ratio by modifying the `repeat` parameter in the JSON file.
...
Therefore, the reproduced results on general VQA (*i.e.*, MMVet, LLaVABench, and MMHal-Bench) may be inferior to [our released model](https://huggingface.co/OpenGVLab/InternVL2-8B-RPO).
Nevertheless, we are confident that you can successfully reproduce comparable performance on all other benchmarks.

We recommend incorporating additional general VQA data to preserve the general VQA abilities, following [our continuation-based data construction pipeline](#generate-more-preference-data).

## Prepare Your Customized Training Data

If you want to prepare your customized preference data, please create a JSON file similar to [this example](https://huggingface.co/datasets/OpenGVLab/MM-Reasoning/blob/main/meta.json).
...
```json
{
  "scienceqa_multi_choice_en_20240402_extracted_pairs_vqa_format_rules": {
        "root": "MM-Reasoning/images/ScienceQA",
        "annotation": "MM-Reasoning/annotations/scienceqa_multi_choice_en_20240402_extracted_pairs_vqa_format_rules.jsonl",
        "data_augment": false,
        "repeat_time": 1,
        "length": 66457
    },
...
Our suggestion is to add new domain-specific data on top of [MM-Reasoning](https://huggingface.co/datasets/OpenGVLab/MM-Reasoning). This will enhance downstream capabilities while retaining the foundational skills. Of course, you can also choose to fine-tune solely on the new data based on your requirements.
...
```sh
cd internvl_chat
sh shell/internvl2.0_rpo/preference_optimization/internvl2_8b_internlm2_7b_dynamic_res_rpo_full.sh
```
...
```sh
# M3CoT
GPUS=8 sh evaluate.sh ckpt/InternVL2-8B-RPO m3cot --dynamic --cot
# MathVista
GPUS=8 sh evaluate.sh ckpt/InternVL2-8B-RPO mathvista-testmini --dynamic --cot
# POPE
GPUS=8 sh evaluate.sh ckpt/InternVL2-8B-RPO pope --dynamic --cot
```
...
To constrct additional open-ended VQA preference data, you can use our [continuation-based pipeline](https://github.com/OpenGVLab/InternVL/tree/main/internvl_chat/tools/mm_reasoning_pipeline/internvl_lmdeploy_continue_wo_image.py) with the following command:
